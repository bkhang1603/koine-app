...
export default function CustomCourseScreen() {
    ...
    const handleCustomCourse = async () => {
        try {
            ...
            setProcessing(true);
            const selectedChapterIds = selectedChapter.map(
                (chapter) => chapter.id
            );
            const bodyData = {
            ...
            Alert.alert(
                "Lỗi",
                `Tạo khóa học tùy chỉnh không thành công ${error}`,
                [
                    {
                        text: "Trang chủ",
                        onPress: async () => {
    ...
    const getSelectedCourse = () => {
        return courseElement?.data.find(
                ...
                >
                    <View className="px-5 py-4">
                        ...
                        <Text className="text-gray-600 mt-1 mb-6">
                            Chọn các chương từ khóa học của chúng tôi để tạo nên
                            trải nghiệm học tập phù hợp với bạn
                        </Text>
                        ...
                        <View className="bg-white rounded-xl p-4 mt-2 shadow-sm">
                            ...
                            {selectedChapter.length == 0 ? (
                                ...
                                <View className="space-y-5">
                                    {Object.keys(groupedChapters).map(
                                        (courseId) => (
                                            <View
                                                key={courseId}
                    ...
                    <View className="mx-4">
                        <View className="bg-white p-4 rounded-xl shadow-lg">
                            ...
                            <Text className="text-gray-600 mb-4">
                                Vui lòng chọn một khóa học để xem và chọn các
                                chương
                            </Text>
                        ...
                        <View className="bg-white p-4 rounded-xl shadow-lg">
                            ...
                            {selectedCourseId && (
                                <View className="bg-indigo-50 rounded-lg p-3 mb-4 flex-row items-center">
                                    ...
                                    <Text className="text-indigo-600 text-xs">
                                        {getChaptersFromSelectedCourse().length}{" "}
                                        chương
                                    </Text>
                                </View>
                            )}
                            ...
                            >
                                <View className="space-y-3 p-1">
                                    {getChaptersFromSelectedCourse().map(
                                        (chapter) => (
                                            <TouchableOpacity
                                                key={chapter.id}
                                                className={`p-3 rounded-xl border ${
                                                    selectedChapter.some(
                                                        (c) =>
                                                            c.id === chapter.id
                                                    )
