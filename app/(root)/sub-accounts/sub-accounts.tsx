...
export default function SubAccountsScreen() {
            ...
            <View className="flex-1 bg-[#f5f7f9]">
                ...
                <View className="flex-1 items-center justify-center p-4">
                    ...
                    <Pressable
                        ...
                        onPress={() =>
                            router.push("/(root)/sub-accounts/create")
                        }
                    >
    ...
    const totalDifferentAssigned =
        myCourse?.data.details?.reduce(
            (sum, item) =>
                sum +
                item.assignedTo.filter((a) => a.id !== profile?.data.id).length,
            0
        ) ?? 0;
        ...
        <View className="flex-1 bg-[#f5f7f9]">
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View className="p-4 mx-4 bg-white rounded-xl shadow-sm mb-6">
                    ...
                    {childs.map((account) => {
                        const activeCourses =
                            myCourse?.data.details.filter((course) =>
                                course.assignedTo.some(
                                    (assigned) => assigned.id === account.id
                                )
                            ) ?? [];
                                ...
                                ? Math.round(
                                      activeCourses.reduce((sum, course) => {
                                          const assignedToThis =
                                              course.assignedTo.find(
                                                  (a) => a.id === account.id
                                              );
                                          return (
                                              sum +
                                              (assignedToThis &&
                                              "completionRate" in assignedToThis
                    ...
                    {/* Learning Stats */}
                    <View className="bg-gradient-to-r from-blue-500 to-blue-600 mt-2 py-4 px-4 rounded-xl">
                        <View className="flex-row items-center">
