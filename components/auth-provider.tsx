...
import { useCheckRefreshMutation, useRefreshAccessMutation } from '@/queries/useAuth'
  ...
  const getNewAccessToken = async () => {
    try {
      ...
      if (currentRefreshToken) {
        const res = await refreshAccess.mutateAsync({ refreshToken: currentRefreshToken.refreshToken })
        if (res?.data) {
          const newAccess = {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      try {
        ...
        console.error('Failed to load token and user from secure storage: ', error)
      }
    }
    initializeAuth()
        ...
        intervalRef.current = setInterval(async () => {
          ...
          const checkRefreshToken = async () => {
            if (currentAccessToken && currentRefreshToken) {
              const res = await checkRefresh.mutateAsync({ accessToken: currentAccessToken.accessToken })
              if (res?.data) {
